...
`npm` 拥有以下的脚本钩子：对于任何在 `package.json` 的 `scripts` 字段中定义的命令，可以通过 `pre` 以及 `post` 作为命令名前缀，额外定义该任务执行前、后的分别执行的钩子命令。例如：
...
命令 `my-task` 是某个目标任务名；在使用 `npm` 或 `yarn` 执行该任务（`npm run my-task` 或者是 `yarn my-task`）时，总是会自动的先去查找该任务是否有 `pre` 任务（即 `premy-task`），如果有就先执行该先决任务，且成功后再执行原任务 `my-task`；如果此任务成功结束了，总是会自动的去查找该任务是否有 `post` 任务（即 `postmy-task`），如果有就执行该后续任务。这种命令钩子，同样也能作用于一些 npm 客户端自身的行为，例如 `install`，`uninstall` 等。

于是，可以尝试用命令钩子来解决以上的 npm 客户端差异的问题，对用户当前使用的 `npm` 客户端进行预检：如果是不允许的不合适的 npm 客户端，直接提前给出提示并抛出错误中止后续行为。
